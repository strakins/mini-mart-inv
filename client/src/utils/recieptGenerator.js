...
export const generateReceiptPDF = (sale) => {
  ...
  // Sale details
  doc.setFontSize(12);
  doc.text(`Receipt: ${sale.saleId}`, 20, 30);
  doc.text(`Date: ${new Date(sale.createdAt).toLocaleString()}`, 20, 40);
  doc.text(`Cashier: ${sale.salesAgent?.name || 'N/A'}`, 20, 50);
  ...
  yPosition += 10;
  doc.setFontSize(14);
  doc.text(`TOTAL: $${sale.totalAmount.toFixed(2)}`, 20, yPosition);
